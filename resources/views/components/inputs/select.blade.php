...
<div class="">
    ...
    <div class="flex flex-col space-y-2 w-full">
        <select {{ $attributes->except('class') }} name="{{$name}}" id="{{$name}}" class="select-sm scrollbar-thin scrollbar-thumb-gray-500 scrollbar-track-gray-700 @error( $attributes->wire('model')->value() ) border-error @enderror" title="{{ $tooltip }}">
