                    ...
                    <div class="relative z-0 inline-flex shadow-sm rounded-md">
                        ...
                        @if (!$paginator->onFirstPage())
                            <a wire:click="previousPage" rel="prev" class="relative first:ml-0 text-xs font-semibold flex w-8 h-8 mx-1 p-0 rounded-full items-center justify-center leading-tight border border-solid border-blue-600 bg-white text-blue-600 hover:bg-blue-100" aria-label="{{ __('pagination.previous') }}">
                                <x-bx-chevron-left class="w-4 h-4" />
                            </a>
                            @if ($paginator->currentPage() > 4)
                            <a wire:click="gotoPage(1)" class="relative first:ml-0 text-xs font-semibold flex w-8 h-8 mx-1 p-0 rounded-full items-center justify-center leading-tight border border-solid border-blue-600 bg-white text-blue-600 hover:bg-blue-100 cursor-pointer" aria-label="{{ __('Go to page :page', ['page' => 1]) }}">
                                1
                            </a>
                            @endif
                        @endif
                                ...
                                @foreach ($element as $page => $url)
                                    ...
                                    @if ($paginator->currentPage() > 3 && $page === 2)
                                        <div class="text-gray-800 dark:text-gray-200 flex items-center mx-1">
                                            <x-bx-dots-horizontal-rounded class="w-6 h-6" />
                                        </div>
                                    @endif
                        ...
                        @if ($paginator->hasMorePages())
                            ...
                            <a wire:click="gotoPage({{ $paginator->lastPage() }})" class="relative first:ml-0 text-xs font-semibold flex w-8 h-8 mx-1 p-0 rounded-full items-center justify-center leading-tight border border-solid border-blue-600 bg-white text-blue-600 hover:bg-blue-100 cursor-pointer" aria-label="{{ __('Go to page :page', ['page' => $paginator->lastPage()]) }}">
                                {{ $paginator->lastPage()  }}
                            </a>
                            @endif
